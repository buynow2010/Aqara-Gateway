    ...
    def update(self, data: dict = None):
        ...
        for key, value in data.items():
            ...
            if key == 'vibration' and value != 2:
                data[self._attr] = VIBRATION.get(value, '未知')
                break
            if key == 'tilt_angle':
                data = {'vibration': 2, 'angle': value, self._attr: 'tilt'}
                break
            if key == 'rotate_angle':
                rotation = BUTTON.get(data.get('button', 0), '未知')
                duration = data.get('action_duration', '未知')
                if rotation != '未知':
            ...
            if key == 'action':
                if 'voltage' in data:
                    return
                data[self._attr] = CUBE.get(value, '未知')
                break
            if key.startswith('action'):
                data[self._attr] = key + '_' + CUBE.get(value, '未知')
                break
